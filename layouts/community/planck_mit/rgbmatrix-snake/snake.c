...
void move_snake(void) {
    ...
    if (collides_with_snake(next_cell)) {
        ...
        if (collides_with_food(next_cell) && snake_status.snake_length < SNAKE_LENGTH_MAX) {
            ...
            snake_status.snake_ms_per_move = (snake_status.snake_ms_per_move > SNAKE_MS_PER_MOVE_MIN + delta_ms) ? snake_status.snake_ms_per_move - delta_ms : SNAKE_MS_PER_MOVE_MIN;
            change_food();
        }
